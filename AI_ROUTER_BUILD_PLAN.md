...
air init           创建最小配置
air start          启动网关和管理页面
air check          校验配置并打印问题路径
air ui             打开当前管理页面
air status         查询运行实例状态
air probe          探测 Provider 协议、模型和连通性
air models         输出聚合后的模型列表
air routes         查看或解释路由
air convert        离线进行协议请求/响应转换
air doctor         检查端口、配置、环境变量和 Provider
air version        输出版本与构建信息
```
