...
- Provider 凭据不会出现在错误、请求日志和诊断包中；仅本机管理 API 与 Web 控制台会返回并显示实际值。
- Provider API Key 直接写入本机 YAML；旧版环境变量引用仅作兼容读取，控制台保存时会转为实际值。
...
直接保存的 Key 会存在于 YAML 与备份中，因此只适用于可信的单用户机器。轮换时先在控制台更新配置，验证新 Key 后删除过期备份；任何曾经进入对话、终端历史、日志或版本库的 Key 都必须在供应商侧撤销并重新签发。
