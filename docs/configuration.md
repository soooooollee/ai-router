...
- `listen`：网关地址，默认 `127.0.0.1:12666`。
- `admin_listen`：控制台地址，默认 `127.0.0.1:12667`。
...
`logging.request_history` 控制内存中保留的最近请求数量，未填写时默认为 50。`logging.capture_bodies` 只影响后续新请求是否记录请求与响应正文，不会删除或补录已有日志。
...
主配置与应用配置都使用同一原子写入流程：创建唯一备份、写临时文件、`fsync`、原子 Rename、重新读取校验，并保留最近 10 份应用备份。Codex 的 `config.toml` 与同目录的 `airoute-model-catalog.json` 作为一个备份单元恢复，避免模型配置与能力目录不一致。
