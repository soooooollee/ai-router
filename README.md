...
Web 控制台按“模型接入 → 路由配置 → 应用配置”组织：接入时可用真实请求自动识别 OpenAI、Anthropic、Gemini 及 Qwen/MiMo 模型族；应用配置页用于管理 Claude Code 等本地工具；界面输入的供应商 API Key 直接写入主配置文件，也仍兼容 `${ENV_NAME}` 环境变量引用。
...
Qwen 3.x、SiliconFlow 与 Xiaomi MiMo 示例见 [examples/airoute.qwen3.yaml](examples/airoute.qwen3.yaml)。
...
测试覆盖四协议请求、响应和流式事件的 16 种转换方向，以及网关端到端转换、重试和 Fallback。
